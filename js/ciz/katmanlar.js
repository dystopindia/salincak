...
// Nebula: gökyüzüne ait, 'lighter' ile yumuşak renk lekeleri (yörünge, buz).
// [x, y, yarıçap, renk, alfa] — gece güçlenir. Resimli ve resimsiz dalların
// ortak parçası olduğu için ayrı fonksiyon.
const YORUNGE_NEBULA = [[3,6.4,6.5,'#6A3D8F',.30],[16,4.6,5,'#2B6F8F',.24],[-6,7.2,4.5,'#8F3D6A',.20]];
const BUZ_NEBULA     = [[6,5.8,6,'#2B6F8F',.26],[-4,6.6,4.5,'#3D7F8F',.18]];
function nebula(liste,gece){
  X.globalCompositeOperation='lighter';
  for(const [x,y,r,renk,a] of liste){
...
export function uzak(d,b){
  ...
  // Boyalı katman varsa (ciz/boyali.js): resim bölgenin SABİT silüetlerinin
  // (sırt, şehir, bulut bankası, gezegen kavsi, buz sırtı) yerine geçiyor.
  // HAREKETLİ ya da gökyüzüne ait detaylar kodda kalıyor ve resmin arkasında
  // çiziliyor: dönen dolap, kuşlar, nebula parıltıları, kuyruklu yıldızlar.
  // Resim sabit, bunlar yaşıyor — sahneyi canlı tutan onlar.
  if(boyaliVar(b,'uzak')){
    if(b===0){ X.strokeStyle=enUzak; X.fillStyle=enUzak; donmeDolap(K_ENUZAK,2.0,5.1,1.7,d.t); }
    else if(b===2) kuslar(.11,d.t,renkA('#0D0820',.6));
    else if(b===3) nebula(YORUNGE_NEBULA,gece);
    else if(b===4){ nebula(BUZ_NEBULA,gece); kuyrukluYildizlar(.06,d.t,B.lamba); }
    boyaliCiz(b,'uzak',gece,d);
  }
...
export function orta(d,b){
  ...
  X.fillStyle=renk;
  // Boyalı orta katman; ateşböcekleri ve dönen uydular üstünde kodda kalıyor
  // (yüzen enkaz ve buz kütleleri artık resimde).
  if(boyaliVar(b,'orta')){
    const gece=1-gunEvresi(d,B.gunEtki).gunGuc;
    boyaliCiz(b,'orta',gece,d);
    if(b===1) atesbocekleri(k,d.t,gece,'#D8F07A');
    else if(b===3) uydular(k,d.t,renkA('#9FB4D8',.55));
    return;
  }
...
export function on(d,b){
  ...
  if(boyaliVar(b,'on')){ boyaliCiz(b,'on',1-gunEvresi(d,BOL[b].gunEtki).gunGuc,d); return; }
